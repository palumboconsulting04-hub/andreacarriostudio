...
interface Props {
  ...
  nombre: string;
  // Para entrar directa a "Mis clases" desde la pantalla final.
  email?: string;
...
export default function Paso5Gracias({ iscrizioneId, disciplinaId, nombre, email, preview }: Props) {
  ...
  return (
    <div className="max-w-xl mx-auto px-8 py-20 text-center">
      ...
      <p className="text-base mb-6" style={{ color: "#56423d" }}>
        Nos vemos pronto en el estudio. Tu sitio es <strong style={{ color: "#7d2b13" }}>Mis clases</strong>:
        ahí tienes tus clases, avisas si un día no puedes venir, guardas tus recibos
        y entras al grupo de WhatsApp.
      </p>

      <a
        href={email ? `/mis-clases?entrar=1&email=${encodeURIComponent(email)}` : "/mis-clases?entrar=1"}
        className="inline-flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl text-white font-semibold shadow-lg hover:opacity-90 transition-opacity mb-3"
        style={{ backgroundColor: "#7d2b13", fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}
      >
        Entrar en Mis clases →
      </a>

      <p className="text-xs mb-4" style={{ color: "#89726c" }}>
        Guárdalo en el móvil para tenerlo siempre a un toque.
      </p>
